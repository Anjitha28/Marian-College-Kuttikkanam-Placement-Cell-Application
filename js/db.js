...
class Database {
    ...
    async init() {
        ...
        try {
            ...
            const [
                ...
                this.client.from('central_marian_students').select('*'),
                this.client.from('central_marian_teachers').select('*'),
                this.client.from('central_marian_training_programs').select('*'),
                this.client.from('central_marian_placement_activities').select('*'),
                this.client.from('central_marian_exams').select('*'),
                this.client.from('central_marian_exam_attempts').select('*'),
    ...
    async deleteRecord(tableName, id) {
        ...
        try {
            let error = null;
            if (tableName === 'Students') {
                const res = await this.client.from('central_marian_students').delete().eq('register_number', id);
                error = res.error;
            } else if (tableName === 'Teacher') {
                const res = await this.client.from('central_marian_teachers').delete().eq('phone', id);
                error = res.error;
            } else if (tableName === 'Training Program') {
                const res = await this.client.from('central_marian_training_programs').delete().eq('id', id);
                error = res.error;
            } else if (tableName === 'Activity') {
                const res = await this.client.from('central_marian_placement_activities').delete().eq('id', id);
                error = res.error;
            } else if (tableName === 'Exams') {
                const res = await this.client.from('central_marian_exams').delete().eq('id', id);
                error = res.error;
            } else if (tableName === 'ClassIncharge') {
                const res = await this.client.from('central_marian_class_incharges').delete().eq('class_name', id);
                error = res.error;
            }
    ...
    async clearAllTrainingPrograms() {
        if (this.client) {
            const { error } = await this.client.from('central_marian_training_programs').delete().neq('id', '');
    ...
    async addExamAttempt(attempt) {
        ...
        try {
            const { data, error } = await this.client
                .from('central_marian_exam_attempts')
                .insert([{
    ...
    loadSampleData() {
        this.cache.students = [
            { name: "Arjun Mathew", registerNumber: "MC_CS_01", phoneNumber: "9876500001", mailId: "arjun@mariancollege.org", course: "BCA", department: "Computer Science", class: "1 BCA A", gender: "Male", password: "password", isCoordinator: true },
            { name: "Diya Mariam", registerNumber: "MC_CS_02", phoneNumber: "9876500002", mailId: "diya@mariancollege.org", course: "B.Sc CS", department: "Computer Science", class: "2 BSc CS A", gender: "Female", password: "password", isCoordinator: false },
            { name: "Meera George", registerNumber: "MC_CS_04", phoneNumber: "9876500004", mailId: "meera@mariancollege.org", course: "B.Sc CS", department: "Computer Science", class: "2 BSc CS A", gender: "Female", password: "password", isCoordinator: false },
            { name: "Siddharth Tom", registerNumber: "MC_CM_01", phoneNumber: "9876500005", mailId: "sid@mariancollege.org", course: "B.Com", department: "Commerce", class: "3 BCom B", gender: "Male", password: "password", isCoordinator: false },
            { name: "Ananya Joseph", registerNumber: "MC_CM_04", phoneNumber: "9876500008", mailId: "ananya@mariancollege.org", course: "B.Com", department: "Commerce", class: "3 BCom B", gender: "Female", password: "password", isCoordinator: false }
        ];

        this.cache.teachers = [
            { name: "Dr. Joseph Thomas", phoneNumber: "9876599999", mailId: "mahesh@mariancollege.org", department: "Computer Science", password: "password", isCoordinator: true },
            { name: "Prof. Mary Varghese", phoneNumber: "9876588888", mailId: "priya@mariancollege.org", department: "Commerce", password: "password", isCoordinator: false }
        ];
        ...
        this.cache.trainingPrograms = [
            {
                ...
                registrations: ['MC_CS_01', 'MC_CS_02', 'MC_CM_01'],
                sessions: [{ id: 'sess_1', date: '2026-06-01', time: '10:00 AM', attendance: ['MC_CS_01', 'MC_CS_02'] }],
                batches: [], feedbacks: []
            }
        ];
        ...
        this.cache.placementActivities = [
            {
                ...
                registrations: ['MC_CS_01', 'MC_CS_02'],
                phases: [
                    { id: 'PHS_1', name: 'Technical', completions: ['MC_CS_01', 'MC_CS_02'] },
                    { id: 'PHS_2', name: 'Final', completions: ['MC_CS_01', 'MC_CS_02'] }
                ]
            }
        ];

        this.cache.classIncharges = [
            { className: '1 BCA A', incharge: 'Dr. Joseph Thomas' },
            { className: '3 BCom B', incharge: 'Prof. Mary Varghese' }
        ];
    }
}
